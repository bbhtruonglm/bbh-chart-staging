...
function ToolTipCustom({ content, visible, position }: TooltipState) {
  ...
  useEffect(() => {
    ...
    if (visible && TOOLTIP_REF.current) {
      ...
      /**
       * Kích thước chiều cao của viewport
       */
      ...
      if (ADJUSTED_LEFT < 0) {
        ...
        setTooltipPosition({
          top: ADJUSTED_TOP,
          left: VIEWPORT_WIDTH - TOOLTIP_WIDTH, // Đặt nó ở lề phải
        })
      } else {
      ...
      if (ADJUSTED_TOP < 0) {
        ...
        setTooltipPosition({
          top: position.y + 10, // Đặt tooltip bên dưới, thêm khoảng cách 10px
          left: ADJUSTED_LEFT,
        })
      }
    }
  ...
  return ReactDOM.createPortal(
    <div
      ...
      ref={TOOLTIP_REF}
      className="absolute z-50 min-w-20 bg-color-inverse text-white text-sm rounded-md px-3 py-1.5 max-w-xs shadow-md"
      style={{
