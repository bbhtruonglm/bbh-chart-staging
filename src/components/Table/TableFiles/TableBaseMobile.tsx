...
export const TableBaseMobile = <T,>({
  data,
  columns,
  onIconClick,
  show_icon = false,
  ...
  const getColumnTotal = (column_id: string) => {
    ...
    const NUMERIC_VALUES = VALUES.map(value => {
      /**
      ...
      /**
       * Nếu giá trị không phải là NaN thì trả về giá trị, ngược lại trả về null
       */
    ...
    return NUMERIC_VALUES.length > 0
      ? NUMERIC_VALUES.reduce((sum, value) => sum + value, 0)
      : '-'
  }
                  ...
                  return (
                    <th
                      ...
                      className={`relative border-0 bg-slate-100 px-2 py-2 h-10 sticky left-0 top-0 text-sm ${
                        index === 0
                          ? `font-semibold border-t rounded-tl-lg border-l bg-slate-200 text-left z-20`
                    ...
                    >
                      <div
                        className={`flex h-full group cursor-pointer ${index === 0 && show_icon ? 'justify-between' : index === 0 ? 'justify-start' : 'justify-end'} items-center`}
                      >
                        {flexRender(
                        ...
                        {index === 0 && show_icon && (
                          <span className=" text-blue-600 cursor-pointer">
                            <ArrowTopRightOnSquareIcon className="size-4 font-medium text-black" />
                          </span>
                        )}
                      </div>
                    </th>
                  ...
                  .map((header, index) => (
                    <td
                      ...
                      className={`border-0 px-2 py-2 bg-slate-100 text-right ${
                        getHeaderGroups()[0].headers.length - 2 === index
