import { selectIsPageIncludesFb, selectStaffList } from '@/stores/appSlice'
...
export const useColumns = ({ is_open_modal }: { is_open_modal?: boolean }) => {
   ...
   */
  const COLUMN_STORAGE_KEY = 'staff_visible_columns'
  ...
  function getName(id: string): string {
    /** Nếu có trong danh sách nhân viên thì trả về tên, ngược lại trả về id */
    if (STAFF_LIST[id]) return STAFF_LIST[id]?.name || id
    /** Trả về id nếu không có trong danh sách nhân viên */
    return id
  }
  /**
  ...
  }, [is_mobile, STAFF_LIST])
  /** COLUMNS_STAFF */
  const COLUMNS_STAFF = useMemo(() => {
    const COLUMN = [
      COLUMN_HELPER.accessor('staff_id_column', {
        meta: {
          label: t('staff'),
        },
        header: () => (
          <HeaderWithTooltip
            label={t('staff')}
            onHover={(e: any) =>
              handleMouseEnterNoTruncated(e, t('staff_desc'))
            }
        ...
        cell: info => (
          <div className="truncate">
            {info.getValue() ? getName(info.getValue()) : '-'}
          </div>
        ),
  ...
  }, [STAFF_LIST, is_mobile, IS_PAGE_INCLUDES_FB])
